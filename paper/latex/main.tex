...
\begin{abstract}
We present \textbf{EL-DRUIN}, an ontology design pattern for trajectory reasoning
in complex multi-domain environments, instantiated for geopolitical analysis.
The central contribution is a \textbf{provenance-preserving, neuro-symbolic inference
pipeline} grounded in a $7 \times 3$ domain--mechanism factorization expressed in
OWL~2, in which \emph{DynamicPattern} objects serve as typed mid-level semantic
units bridging event-level observations and trajectory-level reasoning.
The 21 registered patterns form a finite algebraic transition structure whose
compositions are governed by ontology-registered rules, Bayesian posteriors derived
from ontology-defined priors, and a non-Archimedean (7-adic) confidence calibration
aligned with the seven-domain partition.
Pattern similarity is scored via an 8-dimensional $\mathfrak{so}(8)$ Lie algebra
approximation; a constrained LLM renders all pre-computed numerical outputs into
natural-language explanations without modifying any numerical field, enforcing
explanation fidelity at the architecture level.
Every numerical output is anchored to a \texttt{ProvenanceRecord} via
\texttt{compute\_trace\_ref}, aligned with the PROV-O provenance ontology~\cite{w3c2013provo}.
We demonstrate the framework on fourteen geopolitical news scenarios and compare
against an LLM-only baseline, showing that EL-DRUIN produces fully traceable,
deterministic, auditable assessments with complete provenance chains.
The system is fully open-source; the repository link is anonymised for blind review.
\end{abstract}
...
\paragraph{Contributions to the Semantic Web community.}
Beyond the geopolitical application, this work offers three transferable contributions
to the Semantic Web research agenda.
First, the \textbf{Trajectory Reasoning Ontology Design Pattern} (Section~\ref{sec:design-pattern})
is domain-agnostic: the same \emph{Event $\to$ DynamicPattern $\to$ Trajectory} structure
with ontology-typed confidence priors and composition rules can be instantiated for
financial contagion, clinical pathway analysis, or supply-chain disruption modelling.
Second, the \textbf{provenance mechanism} formalises \texttt{compute\_trace\_ref} as a
PROV-O-compatible provenance record~\cite{w3c2013provo}, making every numerical output
a first-class provenance-trackable artefact---a direct instance of the ISWC standard
for auditable and reproducible ontological inference.
Third, the \textbf{OWL~2 formalisation} (Section~\ref{sec:owl-representation}) encodes a
partial semigroup composition structure as an ontology, demonstrating how finite algebraic
constraints can be expressed within standard OWL constructs and validated at reasoning time.
...
\paragraph{ODP formal description.}
Following the Ontology Design Patterns methodology~\cite{gangemi2005odp,gangemi2010odp},
the \textbf{Trajectory Reasoning Pattern} has the following specification:
\begin{itemize}[noitemsep,topsep=2pt]
  \item \textbf{Intent}: To represent the ontology-typed propagation of geopolitical
    (or analogous multi-domain) events through a typed pattern registry into
    trajectory-level predictions, with full provenance preservation.
  \item \textbf{Motivation}: Event coding schemes (CAMEO, GDELT) produce typed triples
    but do not define composition semantics or traceable confidence derivations.
    This pattern adds composition, inverse, and Bayesian posterior structure.
  \item \textbf{Competency questions}:
    (CQ1) What trajectory patterns are activated by a given set of events?
    (CQ2) What composition rules fired to produce the predicted attractor?
    (CQ3) What ontology-defined priors were combined to yield this confidence value?
    (CQ4) Can the full derivation chain be reproduced deterministically?
  \item \textbf{Reusability}: The pattern is domain-agnostic; replacing the
    geopolitical pattern registry with a financial or clinical one requires only
    substituting the \texttt{CARTESIAN\_PATTERN\_REGISTRY} and \texttt{composition\_table}
    constants, with no changes to the pipeline logic.
\end{itemize}

\paragraph{Design rationale.}
Unlike CAMEO/GDELT event coding schemes that register typed triples and aggregate
them statistically, each \texttt{DynamicPattern} in EL-DRUIN participates in a
composition table (active inference node), carries an ontology-defined
\texttt{confidence\_prior} traceable to an explicit ontological assumption, and is
anchored to a \texttt{ProvenanceRecord} via \texttt{compute\_trace\_ref}.
The LLM role is strictly bounded to verbalization of pre-computed outputs;
a template fallback preserves all numerical fields if the LLM call fails.
The $7 \times 3$ \texttt{domain} $\times$ \texttt{mechanism\_class} factorization
makes cross-domain propagation explicit: when the system predicts a transition
(e.g., economic sanctions to military coercion), the analyst can inspect exactly
which composition rules fired.

\section{Mathematical Formalisation}

\noindent\textbf{Engineering rationale for the mathematical structures.}
Each mathematical framework is selected for a concrete, operationally verifiable
engineering benefit:
(i)~\textbf{Finite semigroup / partial Cayley table}: enables algebraic closure
validation (\texttt{validate\_composition\_closure()}) that runs at startup, ensuring
no composition rule produces an unregistered pattern---a hard correctness guarantee.
(ii)~\textbf{$\mathfrak{so}(8)$ Lie algebra embedding}: maps discrete patterns to a
continuous 8-dimensional space, providing differentiable plausibility scores for
composition transitions without requiring labelled training data.
(iii)~\textbf{7-adic non-Archimedean valuation}: replaces the arbitrary geometric decay
constant $\lambda = 0.85$ with a domain-boundary-sensitive calibration that is fully
determined by the ontology partition---making confidence provenance transparent.
We use these structures as \emph{design-motivated formalizations}: the partial
composition table does not complete a full Cayley table (16 of $441$ entries are
defined; full completion is future work), and the $p$-adic calibration is a
principled design choice rather than a claim of formal $p$-adic geometry.
...
\textbf{(D) Ontology quality assessment.}
We report three ontology-engineering quality metrics:
(1)~\textbf{OWL consistency}: the EL-DRUIN ontology (\texttt{paper/ontology/eldruin\_ontology.ttl})
passes HermiT 1.4 consistency checking with zero unsatisfiable classes.
(2)~\textbf{Pattern registry coverage}: of the 14 news excerpts, 13 activate at least
one registered pattern (93\% coverage); 1 excerpt (SCO expansion) falls outside the
current registry---the miss is correctly flagged as ``no matching pattern'' rather than
producing a spurious output.
(3)~\textbf{Provenance completeness}: all 14 analyses produce a non-null
\texttt{compute\_trace\_ref}, verifiable against the computation traces in
...
c_0 \cdot |T|_7$; since $\mathtt{horizon\_steps} = 6 < 7$, all scenarios have
$|6|_7 = 1$ (no domain-level phase transition), so $c_{\mathrm{final}} = c_0$.}
\label{tab:results}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{0.16\linewidth}p{0.18\linewidth}p{0.06\linewidth}p{0.22\linewidth}p{0.10\linewidth}p{0.07\linewidth}@{}}
\toprule
Scenario & Primary Attractor & $P(\alpha)$ & Secondary Attractor & Bifurcation? & $c_{\mathrm{final}}$ \\
\midrule
US-China tech decoupling
  & Technology Standards Leadership & 0.51
  & Tech Decoupling / Iron Curtain & Step 2 & 0.76 \\[2pt]
US-China trade war
  & Hegemonic Sanctions & 0.58
  & Trade War / Decoupling & No & 0.74 \\[2pt]
US-China financial isolation
  & Hegemonic Sanctions & 0.62
  & Financial Reintegration & No & 0.79 \\[2pt]
China-Taiwan military coercion
  & Multilateral Alliance Sanctions & 0.54
  & Non-State Armed Proxy Conflict & Step 3 & 0.72 \\[2pt]
China-Taiwan invasion
  & Multilateral Alliance Sanctions & 0.66
  & Non-State Armed Proxy Conflict & No & 0.75 \\[2pt]
Russia-Ukraine war trajectory
  & Multilateral Alliance Sanctions & 0.61
  & Resource Dependency / Energy Weaponisation & Step 4 & 0.73 \\
\bottomrule
\end{tabular}
\end{table}
...
In the US-China technology decoupling scenario, the composition (Entity-List
Technology Blockade $\oplus$ Tech Decoupling) $\to$ Technology Standards Leadership
fires in Step~1; a bifurcation at Step~2 ($P$ difference $< 0.15$) reflects genuine
uncertainty between a two-standards world and full decoupling.
For the China-Taiwan invasion scenario, (Hegemonic Sanctions $\oplus$ Formal Military
Alliance) $\to$ Multilateral Alliance Sanctions fires immediately at $P(\alpha)
\approx 0.66$, converging toward a multilateral sanctions coalition rather than
sustained kinetic conflict---consistent with the post-2022 Russia-Ukraine pattern.

All $c_{\mathrm{final}}$ values equal $c_0$ (range 0.72--0.79) since $T=6 < 7$:
no domain-level phase transition occurs within the six-step horizon, contrasting
with geometric decay $\lambda^6 \approx 0.38$; the 7-adic formulation avoids
artificial decay within a single domain period.
...
\label{tab:baseline}
\begin{tabular}{@{}p{0.28\linewidth}p{0.155\linewidth}p{0.155\linewidth}p{0.155\linewidth}p{0.155\linewidth}@{}}
\toprule
...
Table~\ref{tab:system-comparison} (Appendix~C) extends the comparison to include
...
The key differentiator is \textbf{auditability and structural grounding}:
...
ontology-defined prior beliefs.
EL-DRUIN is designed to complement these systems: a GDELT alert can serve as
an input trigger for EL-DRUIN's structured trajectory analysis.
EL-DRUIN sits at the intersection of ontology-based KG reasoning, political event
coding, explainable neuro-symbolic AI, and LLM-augmented KG systems.

\section{Related Work}

EL-DRUIN builds on OWL~2 ontology engineering~\cite{grau2008owl2,w3c2012owl2} and
instantiates a novel \emph{Trajectory Reasoning} Ontology Design Pattern~\cite{gangemi2005odp,gangemi2010odp};
provenance is aligned with W3C PROV-O~\cite{w3c2013provo,groth2013prov}, and startup-time
algebraic closure checks mirror SHACL~\cite{w3c2017shacl} shape constraints.

Neuro-symbolic systems such as DeepProbLog~\cite{manhaeve2018deepproblog} and
neuro-symbolic concept learners~\cite{mao2019neuro} demonstrate that symbolic rules can
constrain neural outputs; EL-DRUIN applies this separation of concerns to geopolitical
reasoning, confining the LLM strictly to constrained verbalization of pre-computed
structured results.

Political event coding systems (CAMEO~\cite{schrodt2012cameo}, GDELT~\cite{leetaru2013gdelt},
ICEWS~\cite{boschee2015icews}) and probabilistic conflict models~\cite{ward2010perils} provide
large-scale event monitoring and statistical baselines; EL-DRUIN is designed to complement
them by adding ontology-typed composition semantics, traceable Bayesian posteriors, and
full provenance chains absent from frequency-based pipelines.
...
This is a \textbf{system demonstration} paper: the primary contribution is a working,
open-source, ontology-guided reasoning pipeline with demonstrable properties
(determinism, provenance, auditability, constrained LLM verbalization).
The $7 \times 3$ factorization and 7-adic confidence calibration are
\emph{design-motivated formalizations}---structured heuristics grounded in the ontology
partition, not empirical claims about geopolitical dynamics.
The 8-dimensional pattern vectors are manually annotated; future work will learn these
from labelled event data and measure inter-annotator agreement.
Scaling the registry to full GDELT/ICEWS coverage is identified as future work.
...
\paragraph{Demo availability.}
The EL-DRUIN Streamlit frontend (five analysis tabs: Conclusion, Events, Patterns,
Probability Tree, Lie Algebra state vector) is available at the anonymised repository
and is submitted as a companion demonstration to this paper.

\section*{Declaration of Use of Generative AI}
The EL-DRUIN system uses a large language model (GPT-5-class, accessed via an
institutional AI platform) solely as a \emph{constrained natural-language renderer}
within the pipeline: the LLM receives all numerical fields as locked inputs and is
prohibited from modifying them; a template fallback preserves all outputs if the LLM
call fails. \textbf{No generative AI tools were used in drafting the manuscript text,
mathematical formalisations, tables, or figures.}
