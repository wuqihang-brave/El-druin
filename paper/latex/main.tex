...
differs in three ways. First, each \texttt{DynamicPattern} participates in a
composition table that defines how pairs of active patterns interact to produce
derived patterns, elevating patterns from database entries to active inference
nodes. Second, each pattern carries an ontology-defined \texttt{confidence\_prior}
that participates directly in the Bayesian posterior computation, making every
confidence value traceable to an explicit ontological assumption rather than a
learned parameter. Third, every numerical output is anchored to a
\texttt{ProvenanceRecord} via \texttt{compute\_trace\_ref}, satisfying the ISWC
standard for auditable and reproducible ontological inference.

\paragraph{Why LLM is needed, and where.}
The LLM plays a strictly bounded role: it renders pre-computed structured output
into 2--3 sentences of natural-language strategic interpretation, and is
explicitly prohibited from modifying any numerical field. All computation—pattern
activation, composition, confidence scoring, attractor detection—is entirely
determined by the ontology and the symbolic pipeline; if the LLM call fails, a
template fallback produces \texttt{conclusion.text} without degrading any
numerical output.
...
fires in Step~1; a bifurcation at Step~2 ($P$ difference $< 0.15$) reflects
genuine uncertainty between a two-standards world and full decoupling---the
empirically contested outcome of US-China tech competition. For the China-Taiwan
invasion scenario, (Hegemonic Sanctions $\oplus$ Formal Military Alliance) $\to$
Multilateral Alliance Sanctions fires immediately, producing $P(\alpha) \approx 0.66$;
the system converges toward a multilateral sanctions coalition rather than sustained
kinetic conflict, consistent with the post-2022 Russia-Ukraine pattern.
Regarding confidence calibration, all $c_{\mathrm{final}}$ values equal $c_0$
(range 0.72--0.79) since $T=6 < 7$: no domain-level phase transition occurs
within the six-step horizon, contrasting sharply with the previous geometric decay
$\lambda^6 \approx 0.38$; the 7-adic formulation avoids artificial decay within
...
\subsection{Knowledge Representation and Political Event Coding}

Formal ontologies represented in OWL 2~\cite{grau2008owl2,w3c2012owl2} provide the
foundation for semantic web reasoning. Large knowledge graphs such as
DBpedia~\cite{bizer2009dbpedia}, Wikidata~\cite{vrandecic2014wikidata}, and
YAGO~\cite{suchanek2007yago} demonstrate the scale at which entity-relation triples
can be curated and queried. Hogan et al.~\cite{hogan2021knowledge} survey knowledge
graph construction and reasoning; Ji et al.~\cite{ji2021survey} cover embedding
methods including RotatE~\cite{sun2019rotate}. Temporal knowledge
graphs~\cite{trivedi2017know,lacroix2020tensor,li2021temporal} and Allen's interval
algebra~\cite{allen1983maintaining} address relational change over time; EL-DRUIN
instead applies ontology-registered composition rules to derive forward-projected
attractor states without requiring labelled training data.

CAMEO~\cite{schrodt2012cameo} provides the standard controlled vocabulary for
political event types; GDELT~\cite{leetaru2013gdelt} and
ICEWS~\cite{boschee2015icews} build on CAMEO to enable large-scale statistical
monitoring but do not model pattern composition or produce provenance traces.
Probabilistic graphical models for conflict prediction~\cite{ward2010perils} require
labelled training data and lack symbolic interpretability; EL-DRUIN's inference is
entirely symbolic and fully auditable.

\subsection{Neuro-Symbolic AI and LLM--KG Integration}

The neuro-symbolic AI programme~\cite{besold2017neural,garcez2020neural} combines
neural pattern recognition with symbolic reasoning; DeepProbLog~\cite{manhaeve2018deepproblog}
...
Agent-OM~\cite{fathallah2023agentom} and LLMs4OM~\cite{babaei2024llms4om} apply LLM
agents to ontology matching. Maedche and Staab~\cite{maedche2001ontology} established
ontology learning from text; chain-of-thought prompting~\cite{wei2022chain} motivates
EL-DRUIN's structured prompts with locked fields. Noy et al.~\cite{noy2019industry}
highlight provenance tracking in industry-scale KGs---a property EL-DRUIN builds in
at the ontology layer. Unlike these works, EL-DRUIN uses the LLM exclusively for
constrained verbalization of pre-computed structured results, ensuring full
...
The \textbf{$7 \times 3$ factorization} and the \textbf{non-Archimedean confidence
calibration} are both design-motivated formalizations. The factorization is an
organizational choice that provides reasoning clarity and calibration structure, not
an empirical claim that geopolitical reality has exactly 21 fundamental patterns; the
7-based valuation is a principled alternative to the ad hoc constant $\lambda = 0.85$,
motivated by the domain partition, not a claim of formal p-adic geometry. The
\textbf{Lie algebra approximation} provides a continuous plausibility score for
...
labelled event data and measure inter-annotator agreement. The composition table
covers only 16 of 441 possible pattern pairs ($\approx 3.6\%$); forward simulation
results should be interpreted as indicative until completeness is substantially
improved, ideally by inferring rules from historical event co-occurrence data.
The pattern registry, domain assignments, and confidence priors are all manually
curated---a rigorous evaluation would require independent expert validation.
The current evaluation covers 14 news excerpts and 8 backtesting events; scaling
to larger independently curated datasets and incorporating human evaluation of
explanation quality is a priority for future work.
