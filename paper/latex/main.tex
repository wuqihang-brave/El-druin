...
The entity type set $E$ contains 18 elements spanning five categories: geopolitical actors (\texttt{STATE}, \texttt{ALLIANCE}, \texttt{PARAMILITARY}, \texttt{IDEOLOGY}), economic actors (\texttt{FIRM}, \texttt{FINANCIAL\_ORG}, \texttt{RESOURCE}, \texttt{CURRENCY}, \texttt{SUPPLY\_CHAIN}), technological (\texttt{TECH}, \texttt{STANDARD}), social-cognitive (\texttt{PERSON}, \texttt{MEDIA}, \texttt{TRUST}, \texttt{INSTITUTION}), and event types (\texttt{CONFLICT}, \texttt{NORM}), plus \texttt{UNKNOWN}. The relation type set $R$ contains 20 elements across four mechanism categories: coercive (\texttt{SANCTION}, \texttt{MILITARY\_STRIKE}, \texttt{COERCE}, \texttt{BLOCKADE}), cooperative (\texttt{SUPPORT}, \texttt{ALLY}, \texttt{AID}, \texttt{AGREE}), dependency/flow (\texttt{DEPENDENCY}, \texttt{TRADE\_FLOW}, \texttt{SUPPLY}, \texttt{FINANCE}), and structural (\texttt{SIGNAL}, \texttt{PROPAGANDA}, \texttt{LEGITIMIZE}, \texttt{DELEGITIMIZE}, \texttt{REGULATE}, \texttt{STANDARDIZE}, \texttt{EXCLUDE}, \texttt{INTEGRATE}). Full enumerations are provided in Appendix~A.
...
The ontology schema is formally expressed in OWL~2 \cite{grau2008owl2,w3c2012owl2} using \texttt{owl:Class} declarations for the eight core classes, \texttt{owl:ObjectProperty} for six object properties (including \texttt{inversePattern} as \texttt{owl:SymmetricProperty}), and 21 \texttt{owl:NamedIndividual} instances of \texttt{DynamicPattern}. The 16 composition rules are encoded as \texttt{CompositionRule} individuals using standard OWL~2 reification (ternary assertions via \texttt{sourcePattern}, \texttt{contextPattern}, \texttt{resultPattern}), compatible with SPARQL reasoning and Prot\'{e}g\'{e} visualisation. A complete Turtle serialisation is provided in \path{paper/ontology/eldruin_ontology.ttl} in the repository; full OWL construct listings are in Appendix~A.
...
\textbf{US-China Technology Decoupling.} The composition (Entity-List Technology Blockade $\oplus$ Tech Decoupling) $\to$ Technology Standards Leadership fires in Step~1. A bifurcation at Step~2 ($P$ difference $< 0.15$) reflects genuine uncertainty between a two-standards world and full decoupling---the empirically contested outcome of US-China tech competition.

\textbf{China-Taiwan Invasion.} (Hegemonic Sanctions $\oplus$ Formal Military Alliance) $\to$ Multilateral Alliance Sanctions fires immediately, producing $P(\alpha) \approx 0.66$. The system predicts convergence toward a multilateral sanctions coalition rather than sustained kinetic conflict, consistent with the post-2022 Russia-Ukraine pattern.

\textbf{Confidence Calibration.} All $c_{\mathrm{final}}$ values equal $c_0$ (range 0.72--0.79) since $T=6 < 7$: no domain-level phase transition occurs within the six-step horizon. This contrasts with the previous geometric decay $\lambda^6 \approx 0.38$; the 7-adic formulation avoids artificial decay within a single domain period.

Extended numerical comparison between 7-adic and geometric-decay calibration at horizons $T \in \{7, 14\}$ is provided in Appendix~B, illustrating the structurally punctuated behavior of the non-Archimedean formulation.
