...
\label{tab:patterns}
\small
\begin{tabular}{@{}p{0.22\linewidth}p{0.10\linewidth}p{0.20\linewidth}
                   p{0.05\linewidth}p{0.30\linewidth}@{}}
\toprule
...
Ontology Design Patterns~\cite{gangemi2005odp,gangemi2010odp} provide reusable
modelling solutions for recurring ontological problems; EL-DRUIN instantiates a
novel \emph{Trajectory Reasoning Pattern} in this tradition.
The W3C PROV-O ontology~\cite{w3c2013provo,groth2013prov} defines a standard model
for provenance; EL-DRUIN's \texttt{compute\_trace\_ref} mechanism is designed to be
PROV-O-compatible, mapping \texttt{DynamicPattern} activations to \texttt{prov:Activity}
and confidence posteriors to \texttt{prov:wasGeneratedBy} chains.
Constraint validation via SHACL~\cite{w3c2017shacl} complements EL-DRUIN's
startup-time algebraic closure checks: \texttt{validate\_inverses()} and
\texttt{validate\_composition\_closure()} enforce structural integrity analogously
to SHACL shape constraints over OWL graphs.
...
Noy et al.~\cite{noy2019industry} highlight provenance tracking in
